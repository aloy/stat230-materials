...
1. Predicting the **mean response** at a specific value of $x$
    ...
    e.g., the average sales price of a car with 40,000 miles
...
2. Predicting the response for a **specific future observation** 
    ...
    e.g., predicting the sales price of your car, which has 40,000 miles
...
Best estimate $\widehat{y}_i = \widehat{\beta}_0 + \widehat{\beta}_1 x_i$
...
- confidence interval for $\widehat{E}(Y|X)$

- prediction interval for $\widehat{y}$


## Standard errors

$\text{SE}(\widehat{E}(Y|X)) = \widehat{\sigma} \sqrt{\dfrac{1}{n} + \dfrac{(x_0-\overline{x})^2}{\sum_{i=1}^n (x_i - \overline{x})^2}}$
...
- The R tutorial is linked on Moodle, but can also be found at: ADD URL
