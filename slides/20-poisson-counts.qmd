...
old_dat <- data.frame(x=(x=runif(500, 3, 40)),
                  y=rnorm(1000, (4.63127 + -0.06338*x)^2, sd = 4*sigma(ols)))
...
ggplot(old_dat, aes(x, y)) +
  ...
  geom_function(fun = \(x) (4.63127 + -0.06338*x)^2) +
    stat_slab(aes(x = x, ydist = "norm", arg1 = yhat, arg2 = 4*sigma(ols)), data = old_slab_dat, alpha = 0.4, inherit.aes = FALSE, scale = 0.6, fill = "#f15a31", side="left") +
  theme_bw() +
...
In our example:  $\log(\widehat{\lambda}) = 3.128 - 0.036 \text{distance}$

. . .

$e^{-0.036} \approx 0.965$

<br>


A 1 meter increase in the distance from the cliff edge is associated with a decrease in the average number of nests per 9m<sup>2</sup> quadrat by a factor of 0.965 
(i.e., a 3.5% decrease).
